...
while True:
    ...
    # Settin Everyting Up (Boards, Who go first, Choose Marker (X,O))
    theBoard = [' '] * 10
                
    player1_marker, player2_marker = player_input() # Choose player marker
    
            
    turn = choose_first()  # Randomly decide  first player
    ...
    while game_on:
        ...
        if turn == 'Player 1':          # Player1's turn.
            ...
            position = player_choice(theBoard) # Choose Position
            place_marker(theBoard, player1_marker, position)   # Place the Marker in the Position

            
            if win_check(theBoard, player1_marker):   # Check if player_1 win
                display_board(theBoard)
                print('Congratulations! Player1 have won the game!')
                game_on = False
            else:                                     # Check tie game condition
